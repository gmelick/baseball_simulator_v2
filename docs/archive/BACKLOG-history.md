...
`SIM_CATCHER_BLOCK_SIGMA=0` (the got-away resolution stays ON) and `SIM_PITCH_CELL_INDEX=1`
(`SIM_PITCH_MIN_CELL=20`); the lane's production set (`tests/acceptance/conftest.py`) matches,
with `SIM467_LANE_RECEIVING=1` / `SIM467_LANE_CELL_INDEX=0` as the arms for the old
configuration. The app was recreated and pre-warmed six workers in 39 s. **Live
`GET /api/games/{pk}/simulate?n_iterations=100`, warm:** 31.4 s and 30.8 s on two games
(the first request after boot 55.0 s, paying each worker's first-touch caches); before the
flip 81-90 s. That is the cell-index ticket's exit criterion AT the 30-second line, not under
it — the residue is the parent-side aggregation and the ~3 effective workers of the fan-out
measured on 2026-09-07 (plan §4), which is the next perf item, not a sampler item. This is
the configuration lane 2 certified (every band passes except strikeouts −2.1% against a 2.0%
floor, a fit residual the redesign's part F owns).
...
re-measure and any edge read built on it — until every band the lanes grade is inside its range.**
Today that means: the walks-per-plate-appearance band (red under the receiving kernel), runs on the
defense-aware configuration (the run-grading question, SIM-520), and the identity-kernel confound
(SIM-523) that drives both. The calibration payoff (SIM-429) keeps the strikeout-prop refit as
its first half; its second half — the CLV re-measure — is gated on an all-green certifying lane.

# 🧹 2026-09-08 — HYGIENE SWEEP 2 (owner-authorized): the catcher receiving profile CLOSED-LANDED; 34 stale rows reconciled to the banners; three fence-resolution design rows PARKED; the datastore-backup gap FILED as SIM-525 (next free ID → SIM-526)

**The catcher receiving profile (SIM-517) is CLOSED-LANDED** — everything it listed is in
production; its residuals live on the identity-kernel confound (SIM-523) and the cell-index
ruling (SIM-467). Its row states the costs plainly so the closure never reads as "certified clean".

**The older tables carried "OPEN" on rows the banners had already closed.** Reconciled to the
banner of record, each with the date and the evidence: the transition fielding draw and the
advancement draws (certified 2026-08-20); the hit-by-pitch outcome and the pickoff channel
(certified); the parser batch, the re-sweep and the out-label fix (2026-08-13); the three
recompute defects (2026-08-15/16); the steal opportunity pool and the steal draw (2026-08-17,
certified 2026-08-30); the kernel-bandwidth fit and its "fit the temperatures" twin
(2026-09-03); the harness-inputs fix and the acceptance lane's three blind spots (verified by
seven certifying runs); the two RNG streams, the run-ledger states and the base-state
invariants (test modules in the tree); the three defects the first production lane found
(resolved by the transition draw and the steal draw); the flip deletions (across three
closures); the hit-by-pitch and wild-pitch/passed-ball/pickoff channels (delivered by the
hit-by-pitch outcome, the receiving profile and the pickoff channel — balks stay unsimulated,
~0.02 per team-game). **Merged:** manager small-ball and the real manager profiles into the
real-manager-profiles ticket (SIM-427); the calibration refit + multi-season curve into the
calibration payoff (SIM-429). **Re-scoped:** the cell-occupancy census (SIM-451) — re-run on
the W1 window after the conditioning rebuild. **Parked:** the park-geometry table, the
trajectory model and the fence-resolution stage (SIM-478/479/480) — the fitted park kernel
took the park effect by another route; reopen with a measured case. **Delivered by the cell
index:** thin-cell widening (SIM-475).

**Writing rules (owner directives, now in CLAUDE.md §14):** no unexplained jargon, and no bare
ticket numbers — describe the work, then the ID in parentheses.

| ID | Title | Type | Pri | Size | Depends-on | Status |
|---|---|---|---|---|---|---|
| **SIM-525** | No backup of any datastore — the Postgres raw tables, the DuckDB file and the engine-artifact bundle have no scheduled copy | Ops | P2 | S | — | 🔲 **OPEN — FILED 2026-09-08 (hygiene sweep 2).** The 2026-07-27 remediation plan's phase 0 ("stop irreversible loss") named it; the plan closed as superseded and this was the one item no later ticket carried. A nightly `pg_dump` plus a copy of `/data/baseball_sim.duckdb` and `/data/play_pool/engine_artifacts` to a second disk is the whole deliverable; the re-sweep (~6 h) and the recompute (~5.7 h) are the cost of not having it. |

# ⚡ 2026-09-07 — SIM-467 CODE LANDED + MEASURED: the pitch-draw CELL INDEX cuts a game iteration from 2.62 s to 0.86 s (3.05×); gated OFF pending the 12×500 lane; the live fan-out reads 81-90 s for n=100 (the five-minute figure was not the steady state)

**What landed (gates: ruff + format clean, mypy clean, 24/24 SIM-467 tests + the SIM-518 /
SIM-455 / SIM-517 / SIM-430 / SIM-451 / SIM-402 / SIM-511-512 suites green; the full unit lane
shows only the four pre-existing environmental failures).** The cell algebra moved into
`simulation/filter_cells.py` (the SIM-451 script re-exports it). `FullPoolSampler` gained the
cell path (`SIM_PITCH_CELL_INDEX`, `SIM_PITCH_MIN_CELL`): each plate appearance draws from its
(runners, outs, score band, batting side) cell's 12 count sub-cells, the in-cell weights are
today's weights bit for bit (the same 4-dim kernel over the same rows — `sit_sigma` untouched),
thin sub-cells widen in decision #19's fixed order (band → side → count) below MIN_CELL, every
draw's widening level is counted, an empty base-out cell raises, and a pre-0023 bundle (no
`bat_home`) indexes 1,440 cells instead of 2,880. Plan §4-5 stamped with the numbers.

**Measured (`scripts/sim467_speed_probe.py`, one warm process, production flags):** 2.62 → 0.86
s per iteration (3.05×). The profile confirms the plan: the whole-pool PA assembly was 74% of an
iteration; the new top costs are the steal draw (28%) and the fielding draw (24%). The live
`/simulate` n=100 reads 81 s and 90 s warm at 6 workers (about three effective workers — the
parent-side residue SIM-430 named), so the 30-s target needs the index ON plus that residue:
projected ~27-30 s. **The certifying lane (12×500, now ~90 min) is the next step; the compose
flip to 1 lands in its commit.** The 4×100 A/B (400 game-sims per arm) reads on-vs-off R −1.6%,
H −1.0%, BB +0.8%, K −1.3% (noise) and HR −10% — toward MLB (on −1.3% vs MLB, off +10%); the
lane decides (plan §5.4 / decision point 3).

**LANE VERDICT (12×500, index ON, 1h28m): 81 passed, 4 failed — R −4.2% (the open SIM-520
grading question), home_win_pct UNDERPOWERED (by design), BB_PA −4.6% (−3.6% before the index:
the SIM-523 receiving-kernel cost) and PITCHES_PA +5.5% (NEW). `scripts/sim467_count_probe.py`
attributes the new red to the RECEIVING KERNEL × the hard cell: with the kernel off the cell
path leaves pitches/PA and the 3-2 ball share unchanged. Re-normalizing inside the cell cannot
help — plan decision point 4 merges into SIM-523. Lane #2 (index ON, receiving OFF) is running
for the owner's table; production stays OFF (`SIM_PITCH_CELL_INDEX=0`).**

**LANE #2 VERDICT (12×500, index ON, receiving OFF, 1h39m): 83 passed, 2 failed — home_win_pct
UNDERPOWERED (by design) and K_PA −2.1% against a 2.0% floor; R PASSES, BB_PA PASSES, PITCHES_PA
PASSES, every other band passes. The index certifies on its own; the receiving kernel does not
survive the hard cell (SIM-523, amplified). OWNER CHOICE: the 3× speed with the receiving kernel
parked until SIM-523's redesign, or the fitted kernel without the speed. Production stays OFF
until the ruling; the table is in the plan §5.4.**

# 🔧 2026-09-07 — SIM-518 CODE LANDED: parts 463 / 464-pitch / 465 / 472 BUILT and gated OFF; the SIM-469 rebuild script is READY but BLOCKED by a DuckDB lock (SIM-524 filed; next free ID → SIM-525)

**What landed (gates: ruff + format clean, mypy clean, 28/28 SIM-518 tests + the neighbouring
sampler / artifact / loop / schema suites green).** Migration 0023 (schema v22 → v23) adds
`bat_home`, `pitcher_pitch_count` (pitches thrown BEFORE the plate appearance) and
`times_through_order` (the live helper's own definition) to `sim.pitch_pool`; the builder writes
them (`POOL_BUILDER_VERSION` sim518.1; the two window expressions are module constants
unit-tested against `times_through_order`); the artifact exports them plus the batted-ball pool's
producing-pitch geometry (`{hand}.pgeom.npy`, NaN preserved) and `pitcher_id`; the loader and the
SIM-403b shared-memory seam carry all five, None on an older bundle. Three draw WEIGHTS, each OFF
by default and byte-identical off (the architecture rule): the fatigue kernel on the pitch draw
(`SIM_FATIGUE_PC_SIGMA` / `SIM_FATIGUE_TTO_SIGMA`; mean 1 per count bucket; unknown rows neutral;
the loop snapshots the inputs once per PA), the batting side on the pitch draw
(`SIM_PITCH_HOME_OFF_WEIGHT`, 0.0 = a hard match), and the drawn pitch's similarity on the
batted-ball draw (`SIM_BB_PITCH_SIGMA`, inside the SIM-511 cell; missing-geometry rows neutral).
Plan: `docs/audit/2026-09-04-sim467-518-plan.md` §6 + §8 (stamped).

**NOT run — the SIM-469 pool-only rebuild** (`scripts/sim518_rebuild_pools.py`, ~2 h detached,
migration → rebuild 2023-2026 → independent verification on 500 games → artifact re-export →
loader round-trip). It failed at `duckdb.connect`: the writer lock is held by the app container's
forkserver process (SIM-524), so the rebuild needs `docker compose stop app` for its duration, and
its artifact re-export overwrites the bundle every running lane reads. An owner call — the new
columns reach the pool only after it runs. **Deferred by design:** SIM-461 waits for the SIM-467
widening evidence (plan §8.3). The fits (plan §8) and the certifying lane follow the rebuild.

| ID | Title | Type | Pri | Size | Depends-on | Status |
|---|---|---|---|---|---|---|
| **SIM-524** | The forkserver holds the DuckDB file open — no writer can lock `/data/baseball_sim.duckdb` while the app runs | Ops/Perf | P2 | S | — | 🔲 **OPEN — FOUND 2026-09-07.** A rebuild's `duckdb.connect` fails with "Conflicting lock is held in PID 0". Inside the app container only PID 8 (`multiprocessing.spawn … spawn_main` — the SIM-430 forkserver) holds the file, a descriptor inherited from the parent when the pool started; the uvicorn process itself holds none. Fix: start the forkserver before the boot opens DuckDB, or close inherited descriptors in the forkserver preload. Until then every pool rebuild needs `docker compose stop app` first (the SIM-517 part-B run predates the current app boot). |
...
**The kernel epic is done.** The owner ruled w=0 (2026-08-31) and it landed with the
SIM-412 flip deleted. The road there caught and fixed one real defect: **lane 4 redded
the hit classes (+3.7 to +10.3% per BIP) because the fielder kernel moved balls BETWEEN
positions** (OF share of drawn balls 52.7→57.4%; OF rows reach ~50% vs IF ~8%) — fixed
by **per-position normalization** (mean factor 1 within each position, full-underflow →
neutral; property-tested; commit f0426b3). **Lane 5 (12×500) then read ALL 14 POOL
BANDS PASS** on the full config, and the **full-power lane (12×1,120 = 13,400 decisive
...
Stats API — the API says which games exist, their start times and their status; our
database supplies OUR data on top (simulations, projections, odds). Cards render three
states: PREVIEW (not started — probable pitchers, lineups when published, our projections),
LIVE (in progress — live score/inning/base-out state), FINAL (completed result).** The
2026-08-29 live frontend test (backend up, real browser, a full user pass over slate → game
page → simulate → Data Lab → Similarity) produced the findings consolidated below; the
epic's part (a) implements the ruling and supersedes three of them (final scores on cards,
the stale-empty-state message, doubleheader disambiguation — schedule data carries game
numbers and start times). Context found during the test: the DB had drifted 16 days stale
(nothing schedules ingestion); a manual Aug-14→29 backfill (181 games, 53,798 pitches, 3
stochastic-segfault retries — the SIM-445 crash class) restored it.

| ID | Title | Type | Pri | Size | Depends-on | Status |
|---|---|---|---|---|---|---|
| **SIM-519** | The LIVE SLATE epic: schedule-driven 3-state frontend + ingestion currency + simulate UX | FE/Ops | P1 | L | — | 🔲 **OPEN — each part individually closable.** **(a) Schedule-driven slate (THE RULING):** the slate endpoint queries the MLB Stats API schedule for the requested date and merges our DB's per-game data; three card states (preview / live / final) with start times + doubleheader game numbers; final cards show the SCORE (today they show only team records); the empty state becomes truthful by construction (the API knows off-days). **(b) Live states:** run `pipeline/live/live_ingestion_pipeline.py` as a managed process (compose service) so live game states + published lineups flow; live cards tick via the existing WS change-signal → REST re-fetch design. **(c) Ingestion currency:** a nightly finals job (the `load_date_range` loader on a schedule, wrapped in the segfault-retry loop the backfill needed — the SIM-445 class strikes ~1-in-2 runs); the slate must never silently trail reality again. **(d) Simulate UX:** the run is asynchronous with a persistent pending/progress state (today a reload silently drops it and a re-click starts a DUPLICATE full run), a run-size choice, a cancel, and CACHED results — an identical repeat currently recomputes ~5 minutes; the persistence layer for storing a game's latest sim run exists but is disabled (`REPLAY_PERSISTENCE_ENABLED`), and the linescore/play-by-play panels' "run a simulation to populate it" message is misleading while it is off. **(e) Projections content:** player NAMES, not raw ids ("Player 575929"); the SIM-434 synthetic bullpen rows (negative ids like "Player -15677556" carrying ~4 IP) labeled "Bullpen (generic)" or aggregated until SIM-427 lands real relievers. **(f) Small:** the Data Lab seasons list ordering nit. Perf evidence logged for SIM-467: warm n=100 ≈ 5 min on the 4-season pool (was ~38 s on 3 seasons). |

# 🧹 2026-08-29 — TICKET HYGIENE SWEEP (owner decisions): 8 closed/merged, the SIM-518 conditioning epic filed (next free ID → SIM-519)

**The owner adjudicated the stale and overlapping rows (2026-08-29). Closed as superseded /
refuted / delivered, each with the evidence on its row:** SIM-460 (the W1 window ruling),
SIM-466 (the prev-chain refutation), SIM-470 (the factorized draw is the framework), SIM-471
(delivered; framing → SIM-517), SIM-455 (verified landed in the shipped loop), SIM-436 (the
SLA residue merged into SIM-467 as its exit criterion), SIM-497c (merged into SIM-497a),
SIM-514 (measured green except (c), transferred to SIM-476), SIM-491 (built + data live; the
fits transferred to SIM-476). **SIM-464 stays open by owner decision, re-scoped to its
pitch-pool half and tracked under SIM-518.**

**The consolidated open board after the sweep — one owner per work stream:**
...
**The ruling (2026-08-29, extends the 2026-08-10 architecture rule):** only weights factor
into the draw; once the draw completes, that IS the play — no adjustment of any kind after
it. Any factor not yet in the draw (framing, platoon, home field, …) gets a ticket to enter
as a draw weight; until its weight lands, its post-draw form is OFF.

**Compliance inventory at ruling time:** platoon (`platoon_off_weight`, SIM-413) ✓ already a
draw weight (value un-fitted → the SIM-476 conditional check); home field / park / fielder
quality (SIM-491) ✓ built as draw weights, fits pending, their inert legacy flips already
scheduled for deletion on enable; IBB (SIM-515) ✓ a draw; steal aggression ✓ a weight in
form (the leverage-factor question is SIM-476 step 0, orthogonal). **The ONE violation was
catcher framing (SIM-428)** — a post-draw ball↔called-strike flip, live in production. Done
under this ruling: `SIM_FRAMING` default flipped OFF (code default + the lane's production
pin; the flip is deleted when the SIM-517 weight lands). Bounded effect, pre-measured by the
diagnosis' RAW-vs-FINAL columns: without framing the chain reads BB/PA +1.5% and K/PA −1.0%
vs the pool — inside the 2% pool-band floors; the next certifying lane re-reads it.
Dead-signal cleanups (pitch_out / sac_bunt set-but-never-read, the hit-and-run
steal-suppression trap) stay SIM-427-scope as recorded in the SIM-476 plan.

| ID | Title | Type | Pri | Size | Depends-on | Status |
|---|---|---|---|---|---|---|
| **SIM-517** | The catcher RECEIVING profile as a DRAW weight (framing + the dropped third strike — absorbs SIM-484) | Sim/Data | P2 | M | — | ✅ **CLOSED-LANDED 2026-09-08 (owner decision).** Delivered and in production: the catcher receiving profile with the uncaught-third-strike columns (migration 0021); catcher identity and the got-away flag on every pitch-pool row (migration 0022, pool rebuilt); the receiving kernel fitted at framing 0.25 / block 0.05 inside the pitch draw; the got-away resolution (the absorbed dropped-third-strike work); the framing flip deleted; two new pool bands. NOT certified clean: the 2026-09-04 lane's three reds — walks −3.6%, hit-by-pitch −10.9%, steal attempts +2.5 pp — belong to this kernel, and the 2026-09-07 lanes showed it does not survive the pitch-draw cell index. Production keeps it ON by the owner's decision with those documented costs. The defect — an identity kernel that proxies a catcher's team context — is the identity-kernel confound ticket (SIM-523); whether the kernel is parked for the cell index's 3× speed is the cell-index ruling (SIM-467). Was: 🔲 **OPEN.** Rebuild SIM-428 framing per the ruling: (a) migration 0021 adds `catcher_id` to `sim.pitch_pool` (from `raw.pitches.fielder_2`; columns LAST — the positional-INSERT trap; version file 20→21 + the version test… |
...
**The instruments ran (12 park-balanced games × 150 iters = 1,800 sims + a 12×50 IBB probe;
results: `docs/audit/2026-08-20-sim429-514-diagnosis-results.md`, evidence JSONs beside it).**
The plan's decision tree resolved on a branch it did not enumerate: per-count outcome rates
match the pool within ±0.5pp at every count, count visitation matches MLB within ±0.23pp,
framing is slightly ball-REDUCING — the kernels do not tilt and the count machine does not
mis-transition. The surplus enters elsewhere:

* **IBB 54% (+0.201/tg):** the sim issues 0.3233 intentional walks/team-game vs MLB 0.1224
  (2.64×). `_should_issue_ibb` is a hand-tuned formula (predates the 2026-08-10 architecture
  rule) that re-rolls PER PITCH, compounding toward certainty in every qualifying spot. The
  probe reconciles exactly: box BB = pitched BB + IBB, no residual. → **SIM-515**.
* **Markov structure 33%:** solving the count-machine chain on MLB's OWN per-count rates
  gives 0.0839 BB/PA and 0.2142 K/PA — but real MLB PAs walk 0.0813 and strike out 0.2232.
  Pitches within a real PA correlate beyond the count; the sim (0.0850/0.2160) sits exactly
  where any count-conditioned independent sampler must. **This also closes SIM-514(b): the K
  deficit is the same artifact, not a strikeout defect.** Fixing it means conditioning the
  pitch draw on within-PA context (the pool already carries `prev_pitch_outcome`) — an OWNER
  design decision, not a tuning knob.
* **Pool era 12% + kernel tilt 1%:** the 2024-26 recency mix walks ~1% more than the 2025
  centre (pre-registered here per the SIM-507 precedent — an owner reference question).

**SAME-DAY ADDENDUM (the owner's three questions, answered by measurement — details in the
results doc's addendum):** (1) **prev-pitch conditioning REFUTED** — the (count × prev) chain
on MLB-2025's own data reads BB/PA 0.0839 / K/PA 0.2140, identical to count-only; the
within-PA correlation is whole-PA-scale, so the structural fix is NOT built (the SIM-429
"structure decision" row below is CLOSED — do not condition on `prev_pitch_outcome`).
(2) **OWNER RULING 2026-08-20: the grade is POOL TOTALS** — the sim's frequencies are graded
against the play pool's own totals (supersedes the SIM-508 2025-season references for the
frequency channels). Against the live pool the diagnosis run grades: BB/PA +0.4%, K/PA
+0.3%, HBP +1.2%, pitches/PA +0.1%, DP/opp +1.6%, 3B/BIP −3.1% — the sampler is faithful;
the true remaining reds are IBB volume (SIM-515) and steal att/opp −15%. → **SIM-516** does
the lane re-referencing. (3) **The window census ran** (`scripts/pool_window_census.py`):
centres differ <0.5% across the three options; W1 (full 2023-2026) has the thickest hard
cells (439 vs 323 in the thinnest, +36% BIP rows), is rules-homogeneous (pitch-clock era),
and self-heals season boundaries; the rolling window buys only the 2023 tail and costs
daily-moving artifacts. **Recommendation: W1, pending the owner's confirmation.**

| ID | Title | Type | Pri | Size | Depends-on | Status |
|---|---|---|---|---|---|---|
| **SIM-516** | Re-reference the certification grade to POOL TOTALS + the window rebuild | Test/Data | P1 | M | SIM-515 | 🟢 **OWNER CONFIRMED W1 (full 2023-2026) 2026-08-20; BUILT, certifying lane pending.** Landed: `RECENCY_FLOOR_SEASONS` 3→4 (the export now takes the 4 newest seasons = the last three completed + the current); artifacts re-exported on 2023-2026 (467k BIP; the pitcher-sim matrix re-scored on the same window so 2023 pool rows keep real similarity weights); `bands.POOL_REFERENCES` — 11 per-opportunity bands (BB/IBB/K/HBP/pitches per PA; 1B/2B/3B/HR/ROE per BIP; DP per opportunity) with centres from the window census + `evaluate_pool` (binomial-SE + floor verdicts); the lane conftest gained the opportunity probes (`pool_counts`: PA, pitched-vs-intentional walks, HBP, K, BIP, DP numerator+denominator); the nine superseded box channels (H/HR/2B/3B/BB/K/DP/ROE/ROE_reached) DEMOTED to informational per the ruling — R, SB, CS, home_win_pct stay game-graded (SB/CS until their per-opportunity bands land with SIM-514(c)/476). The original spec below stands as filed. **LANE RUN 1 (12×500, 4h20m, 2026-08-20): 32/37 green — R PASSES on the new window; 9/11 pool bands PASS. The 5 reds: BB_PA +2.9% / IBB_PA −100% = ONE label defect the new band caught (the IBB's event was `walk`; fixed same-day, commit 0c9a775 — corrected arithmetic greens both); SB −7.4% / CS −9.8% (the known SIM-514(c) steal residual, game-graded until its per-opportunity bands land); home_win_pct 0.4848 at 5,990 decisive (fails-by-design below 13,365 — the SIM-491 fit is the open work). The lane re-runs post-fix for the clean certification.** **LANE RUN 2 (12×500, 4h19m, post-fix): ✅ CERTIFIED — ALL 11 POOL BANDS PASS (BB_PA −0.4%, IBB_PA −3.0% with 1,278 IBBs counted, K_PA −0.2%, HBP −0.1%, pitches/PA −0.4%, 1B/2B −0.0%, 3B +0.9%, HR +1.3%, ROE +3.3%, DP/opp +1.3%) and R PASSES −1.2%. Box BB 3.5358 → 3.3318/tg (SIM-515's predicted −0.20 delivered). The informational channels show the era gaps the ruling anticipated (HR +4.4%, 3B +11.3%, BB +5.3% vs 2025) while their pool bands pass — era, not mechanism. The only asserted reds: SB −7.4% / CS −9.8% (SIM-514(c)/SIM-476) and home_win_pct UNDERPOWERED (0.4848 at 5,990; needs 13,365 + the SIM-491 fit). SIM-516 is CLOSED.** |
| *(original spec)* | | | | | | (a) Rebuild the pools + artifacts on the confirmed window (`scripts/sim491_rebuild_pools.py`-style; a season-list change in the export + recency ref). (b) Re-reference `tests/acceptance/bands.py` per the 2026-08-20 owner ruling: the frequency channels grade against the POOL's own totals (per-opportunity bands — BB/K/HBP per PA via the count-machine chain, 1B/2B/3B/HR/ROE per BIP, DP per opportunity, steal attempts per opportunity + safe share), with floors from binomial SEs at lane volume; the lane conftest gains the opportunity-count probes (PA, BIP, DP-opportunity, steal-opportunity denominators — the sim429/sim514 instrument wrappers are the worked pattern). Game-level channels (R, home_win_pct) stay game-graded — they are what the pool cannot state. `scripts/pool_window_census.py` is the derivation instrument (it already prints every centre per window). Update the bands.py history section + the pinned band-arithmetic tests together (the file's own MOVING A BAND rules). | (a) Rebuild the pools + artifacts on the confirmed window (`scripts/sim491_rebuild_pools.py`-style; a season-list change in the export + recency ref). (b) Re-reference `tests/acceptance/bands.py` per the 2026-08-20 owner ruling: the frequency channels grade against the POOL's own totals (per-opportunity bands — BB/K/HBP per PA via the count-machine chain, 1B/2B/3B/HR/ROE per BIP, DP per opportunity, steal attempts per opportunity + safe share), with floors from binomial SEs at lane volume; the lane conftest gains the opportunity-count probes (PA, BIP, DP-opportunity, steal-opportunity denominators — the sim429/sim514 instrument wrappers are the worked pattern). Game-level channels (R, home_win_pct) stay game-graded — they are what the pool cannot state. `scripts/pool_window_census.py` is the derivation instrument (it already prints every centre per window). Update the bands.py history section + the pinned band-arithmetic tests together (the file's own MOVING A BAND rules). |

**SIM-514 verdicts (12×150 decomposition run):** (a) DP per-opportunity 0.1417 vs 0.1391
pool-expected (ratio 1.019) — machinery CLEAN; opportunities/tg 5.37 vs MLB 4.85 (+10.7%
traffic, IBB+BB driven) — closes with the walk fix. (b) K — closed, structural (above).
(c) SB attempts/opportunity at 2B 0.0184 vs the pool's own 0.0217 (−15%), opportunities +9%;
re-measure after the walk fix, then SIM-476 on the 2B attempt weighting if still red.
(d) 3B drawn rate per cell = 0.973× the pool's own — draw CLEAN; the lane's +11.9% is
era (+2.6%) + BIP traffic (+2.3%) + noise; (d)(2) closed by inspection (the stretch draw
fires only on `hit == 2`, nothing relabels to `triple`).
...
# ⚾ 2026-08-19 — SIM-510..512 CERTIFIED (12×471, n=11,304): R/H/HR/2B GREEN; DP fixed (-77% → +8.3%); ROE_reached 0.2165 vs 0.2078 INSIDE; findings filed as SIM-514 (next free ID → SIM-515)

**The drawn row IS the play, certified.** SIM-510 rebuilt + validated at full scale (guard
99.2%; all 24 base-out cells common; 100.0% of DP rows name a retired runner). SIM-511+512
landed as ONE change per the owner's rulings (hard base-out cell, NO widening; five-scenario
advancement draws; `RUN_VALUES` numbers REMOVED — RE24-only ledger, `field_error` its own
canonical outcome). The SIM-500 conservation guard earned its keep: it caught a 1-in-100k
body-loss collision the unit tests missed (fixed — the seat ladder never drops a body), and
two lane probes stale-pinned to the alias world were re-pinned.

**THE CERTIFIED SCOREBOARD (2025 bands, n=11,304):**
* **GREEN: R, H, HR, 2B** — HR (+6.5%) and 2B (+5.2%) were red before the epic; H passes
  without its xfail for the first time.
* **THE EPIC'S TWO TARGETS: DP 0.8081 vs 0.7459 (+8.3%, from -77%); ROE_reached 0.2165 vs
  0.2078 (+4.2%, INSIDE, from 0.0000).** SIM-494 and SIM-496 are closed by measurement.
* **Red, ticketed:** BB +11.7% (SIM-429, parked by the owner); DP +8.3% / K -2.5% / SB -7.6% /
  3B +11.9% → **SIM-514** (below); home_win_pct 0.4735 UNDERPOWERED (the bias is INERT by
  design — SIM-491 rebuilds it as a draw weight; full power needs 13,365 obs).
* **Inside-band, spread-unresolved:** CS -6.7%, ROE +4.3%, ROE_reached +4.2% (sim sd > sd_ref
  persists — the DP/ROE mechanics did not resolve the overdispersion question).
* **The follow-up solutions are documented for a fresh context:**
  **`docs/audit/2026-08-19-sim429-514-491-diagnosis-plan.md`** — SIM-429's per-count diagnosis
  script + decision tree; SIM-514's four decompositions; SIM-491's three draw-weight rebuilds
  (home flag column / park kernel / fielder-quality factor); the lane commands + the traps.

| ID | Title | Type | Pri | Size | Depends-on | Status |
|---|---|---|---|---|---|---|
| **SIM-514** | Post-transition scoreboard findings: DP +8.3%, K -2.5%, SB -7.6%, 3B +11.9% | Sim | P1 | M | SIM-513 | ✅ **CLOSED 2026-08-29 (hygiene sweep 1)** — measured green except the steal attempts, transferred to the kernel fit (closed 2026-09-03). Was: 🔲 **OPEN — diagnosis first, measurement before tuning. The worked plan: `docs/audit/2026-08-19-sim429-514-491-diagnosis-plan.md` §2.** The four reds form one interacting system around traffic: (a) **DP +8.3%** — hypothes… |

# 🏗️ 2026-08-18 — SIM-510→513 FILED: the fielding TRANSITION draw — finalize the loop before tuning statistics (owner design; next free ID → SIM-514)

**The owner's ruling: finalize the simulation loop first; individual-statistic tuning waits.**
**Design + closed decisions: `docs/audit/2026-08-18-sim510-513-fielding-transition-design.md` —
read it before starting; the review decisions there are settled, do not relitigate — three owner
rulings dated 2026-08-19 are AMENDED inline there: NO widening in the fielding draw; all nudge
re-validation lives in SIM-491, not SIM-513; and SIM-511+512 land as ONE combined change (no
feature flag — the station-to-station interim state never reaches `master`).**
**Implementation plan: `docs/audit/2026-08-19-sim510-513-implementation-plan.md`.** The design
in one breath: ONE fielding draw over real transition rows (hard-filtered to the live base-out
state) IS the play — event, outs, WHO is out, all forced/automatic movement — and a runner
advancement draw fires ONLY in five discretionary scenarios (1st→3rd on a single, 2nd→home on a
single, 1st→home on a double, tag-ups, batter stretches), keyed to the runner's speed and the
SPECIFIC fielder's arm. The fielding row's discretionary movement is NORMALIZED to
station-to-station so the advancement draw is the sole authority (the double-count guard).
This supersedes the old SIM-462/473 rows and structurally fixes SIM-494 (phantom DP) and
SIM-496's out-inference; the SIM-349 sac-fly nudge retires with it.

| ID | Title | Type | Pri | Size | Depends-on | Status |
|---|---|---|---|---|---|---|
| **SIM-510** | Transition columns on `sim.outcome_pool` + the advancement opportunity pools | Data | P1 | M | — | ✅ **CLOSED 2026-08-19** (all parts built, rebuilt on all ten seasons, validated at full scale — see the top banner). The spec as filed: (a) DuckDB migration 0018 (v17→v18): pre/post runner identities (`on_*`, `post_on_*`), per-runner scored / out-advancing flags, and who-was-out on `sim.outcome_pool` — the columns exist on `raw.pitches` (the baserunner profiles already read them); columns LAST (positional-INSERT trap; clone `test_the_positional_insert_matches_the_ddl`). (b) Build the five advancement OPPORTUNITY pools from the same rows (non-attempts included — the SIM-468 denominator lesson): per opportunity, attempted / outcome (safe, out, error-extra-base), with runner sprint speed, the row's `fielder_id`/arm, spray/EV/distance, and outs as features. (c) Artifact export + shared-memory publication (clone the steal-pool pattern end to end). Bump `POOL_BUILDER_VERSION`. NO re-sweep needed — pool rebuilds only. |
| **SIM-511** | The transition fielding draw — the drawn row IS the play | Sim | P1 | L | SIM-510 | ✅ **CLOSED-CERTIFIED 2026-08-20** — the 12×500 lane certified the transition fielding draw (all pool bands pass; double plays from −77% to inside the band). Was: 🟢 **LANDED 2026-08-19 with SIM-512 (one combined change); the certification lane adjudicates the bands under SIM-513.** The spec as filed: Replace `_full_pool_fielding`'s event-only draw + out-inference (`outs = 0 if rh… |
| **SIM-512** | The five-scenario advancement draw | Sim | P1 | L | SIM-510, SIM-511 | ✅ **CLOSED-CERTIFIED 2026-08-20** with the transition fielding draw — the five-scenario advancement draws certified in the same lane. Was: 🟢 **LANDED 2026-08-19 with SIM-511 (one combined change).** The spec as filed: Per-runner attempt→outcome draws from the SIM-510 opportunity pools, lead-first with can't-pass occupancy; if the lead does not attempt, no t… |
| **SIM-513** | Retire the legacy paths + certify the finalized loop | Sim/Test | P1 | M | SIM-511, SIM-512 | ✅ **CLOSED 2026-08-19.** The 12×471 lane ran end to end; the scoreboard is read and filed (the top banner; SIM-514 carries the findings). The H/DP/ROE_reached strict xfails are DELETED (their own instruction, validated: H passed, DP/ROE_reached measured real). The SIM-349 sac-fly-intent nudge is RETIRED (commit 18daaac — the SIM-512 tag draw owns sacrifice flies; hit-and-run/sac-bunt stay). The SIM-455/SIM-484 filings are VERIFIED (both claims hold; both stay open; SIM-484's SIM-496-era framing corrected). Scope note, on the record: the legacy full-pool resolution (phantom-DP guard, out-inference, `_full_pool_out_advancement`, `_tag_rate`, `_extra_advance`) survives ONLY as the pre-sim510-bundle fallback the pinned-off CI fixtures exercise — production runs none of it; deleting the fallback entirely means retiring legacy-bundle support, a separate decision. | **NO validation in this ticket (owner ruling 2026-08-19)** — SIM-491 owns re-validating the upstream nudges (SIM-412 home-field vs the 0.5428 centre, SIM-411 park, SIM-425b fielder-RBF) against the new draws, AFTER this ticket lands. Golden-fixture regen (intentional model change), full 12×471 lane on the 2025 bands, re-read the WHOLE scoreboard (DP/ROE fixes move R both ways), and re-check the CS/3B/ROE overdispersion (sim sd > sd_ref) — correct DP/ROE mechanics may be its cause. Also verify-and-close the stale SIM-455/SIM-484 filings while in the file. |
...
| ID | Title | Status |
|---|---|---|
| **SIM-507** | The pickoff channel + the K+CS label | ✅ **CLOSED-CERTIFIED 2026-08-30** — the pickoff channel certified with the three steal bands (the kernel-fit ticket's part 1). Was: 🟢 **BUILT 2026-08-18; certifying lane pending.** DuckDB migration **0017** (v17): `pickoff_out`/`pickoff_advancing`/`pickoff_error` on `sim.steal_opportunity_pool`, labeled from `raw.play_events` per (PA, target) with ou… |
...
| **SIM-501b** | Fix `outs_on_pitch` in the ETL | Data | P2 | M | — | ✅ **CLOSED 2026-08-13** — the re-sweep (2017-2026, 22,533 games) rewrote the data with the fixed parser; nothing reads the column any more. Was: 🟡 **CODE DONE** (landed `4915c16`; counts runner movements, 0.990 of real outs). The swept DATA is still pre-fix until the SIM-488 re-sweep. Nothing reads the column any more (SIM-501c), so this is no longer on any criti… |
...
| ID | Title | Type | Pri | Size | Depends-on | Status |
|---|---|---|---|---|---|---|
| **SIM-494** | Double plays are under-counted about **5×** in the production configuration | Sim | P1 | M | SIM-450 | ✅ **CLOSED-RESOLVED 2026-08-20 by the transition fielding draw** — double plays went from −77% to inside the band (double plays per opportunity +1.3% at certification). Was: 🔲 **OPEN. NEW — no prior ticket, no `CLAUDE.md` entry, no explanation yet.** Measured **0.1600 double plays per team per game against an MLB centre of 0.8160 — 80.4% low**, at 400 production game-sims (n = 800 team-games… |
| **SIM-494 (detection note)** | How to make the guard catch this — analysed in Phase 1, not built | Sim | P1 | S | SIM-494 | ✅ **CLOSED 2026-08-20** — moot with the fix; the base-state invariants and conservation guard (SIM-500, landed) hold the structural check. Was: 🔲 **OPEN.** A review flagged that `runners_retired` is a hardcoded `0` on every in-play ledger call, so the SIM-500 transition guard "cannot detect the phantom double-play runner". The first half is true; the conclusion… |
| **SIM-495** | `_full_pool_steal_decision` is unreachable when `SIM_MANAGER=1` — measured confirmation | Sim | P1 | XS | SIM-468, SIM-474 | ✅ **CLOSED-RESOLVED 2026-08-17 by the steal draw (SIM-474)** — steals are drawn from the opportunity pool; the manager green light is a weight, not a gate. Was: 🔲 **OPEN — CONFIRMS SIM-474. Do not re-diagnose it; SIM-474 is the fix.** This ticket records the measurement, not a new analysis. In the production configuration the lane measured **SB = 0.0000 against 0.59 and CS = 0.0… |
| **SIM-496** | A drawn reach-on-error is converted into an out — the batter is retired and credited a hit | Sim | P1 | S | SIM-453 | ✅ **CLOSED-RESOLVED 2026-08-20 by the transition fielding draw** — the drawn row carries its own outs and destinations, so a drawn reach on error reaches; the reach-on-error band passes. Was: 🔲 **OPEN — CONFIRMS SIM-453.** `_full_pool_fielding` infers outs from the drawn event: `outs = 0 if int(rh) > 0 else 1` (`simulation/sim_loop.py:1432`). A pool `field_error` row carries `result_hits = 0`, so **every draw… |
...
| ID | Title | Type | Pri | Size | Depends-on | Status |
|---|---|---|---|---|---|---|
| **SIM-449** | Harness passes the defense maps + park factor — `_sim_kwargs` drops both, so A/B tests of `SIM_PARK_FACTOR` / `SIM_FIELDER_RBF` compare two identical no-ops | Test/CI | P1 | S | — | ✅ **CLOSED-VERIFIED 2026-09-08 (hygiene sweep 2)** — the harness names its park factor and defense maps on every run (`sim_stats.py` prints both per game) and every certifying lane since 2026-08-20 ran through it; the 2026-08-10 census found all five callers resolving the factor. The one consequence still owed — CLV reads made before 2026-08-10 were park-blind — lives with the CLV re-measure (the calibration payoff, SIM-429). Was: 🟡 **OPEN — BUILT, NOT TRUSTED.** `simulation/sim_kwargs.py` and `tests/unit/test_sim449_sim_kwargs.py` ship the shared builder, and `scripts/sim_stats.py` calls it. An independent review then found the fix **incomplete**… |
| **SIM-450** | Acceptance-band validation lane in the production configuration | Test/CI | P1 | L | SIM-449 | ✅ **CLOSED-DELIVERED 2026-09-08 (hygiene sweep 2)** — the three blind spots are fixed: the round-3 floors reject a 7% run shortfall (the bands rewrite of 2026-08-16), home_win_pct reports UNDERPOWERED instead of passing a coin flip, and the band arithmetic runs in CI on every push. The heavy production lane runs by hand in a container — seven certifying runs since 2026-08-20, the two cell-index lanes of 2026-09-07 among them; a self-hosted runner was never registered and is not needed for that. Was: 🟡 **OPEN — BUILT, NOT TRUSTED. The instrument carries the defect it was built to detect.** The lane exists (`tests/acceptance/`, `.github/workflows/acceptance-nightly.yml`) and its first production run produced SIM-494,… |
| **SIM-451** | Measure the filter cell-occupancy distribution (2,880 cells, at 3 and at 10 seasons) | Data | P1 | S | — | 🟡 **RE-SCOPED 2026-09-08 (hygiene sweep 2).** The second round landed: the script carries a three-way coverage gate and a non-zero exit. The W1-window re-run is still owed, and it matters again after the conditioning rebuild doubles the cells with the batting side; run it then and set the minimum cell size from it (the cell index shipped with 20, and 99.96% of draws sat in their exact cell). Was: 🟡 **OPEN — BUILT, NOT TRUSTED.** `scripts/measure_filter_cells.py`, `tests/unit/test_sim451_filter_cells.py` and a report (`docs/audit/2026-08-10-sim451-filter-cell-occupancy.json`) all ship. An independent review then f… |
| **SIM-498** | Two independent RNG streams — the loop rng and the full-pool rng are built from the same integer | Sim | P2 | XS | — | ✅ **CLOSED-LANDED** — two independent random streams (`tests/unit/test_sim498_rng_streams.py`). Was: 🔲 **OPEN.** `SeedSequence.spawn(2)`. Measure prop spread after. |
| **SIM-499** | Explicit pre/post states on the run-value ledger; delete the conservation derivation | Sim | P1 | M | — | ✅ **CLOSED-LANDED** — explicit pre/post states on the run ledger (`tests/unit/test_sim499_run_ledger.py`). Was: 🔲 **OPEN.** Dissolves 3 tracked defects at once (state-read-after-mutation, DP desync, ROE at 0.00 run value). |
| **SIM-500** | Real base-state invariants + a transition assertion | Sim | P2 | S | SIM-499 | ✅ **CLOSED-LANDED** — real base-state invariants and the conservation guard (`tests/unit/test_sim500_base_invariants.py`; it caught a 1-in-100k body-loss collision in production). Was: 🔲 **OPEN.** `assert_consistent` today rejects only a negative id, yet is called as a correctness guard. |
...
| **SIM-457** | GB/FB/LD rates use an **outs-only** denominator — inflate ~1.4×, biased by pitcher quality | Data | P1 | XS | — | ✅ **CLOSED 2026-08-16** — re-landed on the events-based out label; the all-seasons recompute of 2026-08-13→15 carries it. Was: 🔲 **OPEN.** Detection: the three rates sum to ~1.4, not ~1.0. |
| **SIM-458** | The run-expectancy matrix misses last-plate-appearance runs | Data | P1 | S | — | ✅ **CLOSED 2026-08-16** — the run-expectancy fix re-landed 2026-08-13 and the recompute carries it. Was: 🔲 **OPEN.** Biased low; worst at (2 outs, runner on 3B). Now load-bearing twice. |
| **SIM-459** | Profile recompute (~5.7 h) | Data | P1 | M | 456, 457, 458 | ✅ **CLOSED 2026-08-15** — the all-seasons recompute ran and was verified (2024 median ERA 4.07 vs MLB 4.08; the pool's hit-with-out rows match the raw measurement). Was: 🔲 **OPEN.** One pass for all three. |
| **SIM-460** | Raise the recency floor from **3 seasons to 10** | Data | P1 | XS | SIM-467 | ⛔ **CLOSED-SUPERSEDED 2026-08-29** by the owner window ruling (2026-08-20: the last three COMPLETED seasons + the current one; `RECENCY_FLOOR_SEASONS = 4`, measured by the window census). A future widening needs a fresh measured case against that ruling. |
| **SIM-461** | Make batter hand a **weight**, not a pool partition | ML | P2 | M | SIM-470 | 🔲 **OPEN — tracked under SIM-518 (2026-08-29; the SIM-470 dependency is closed).** ~2× every cell. Validate against SIM-450 bands. |
| **SIM-462** | Runner-identity columns (`on_1b/2b/3b`, `post_on_*`) on `sim.outcome_pool` | Data | P1 | S | — | ⛔ **SUPERSEDED 2026-08-18 by SIM-510** (the fielding-transition epic banner at the top). No pool carries runner identities today. Enabled SIM-473 (also superseded). |
| **SIM-463** | Ten pitch-feature columns + `pitcher_id` into the batted-ball artifact | Data | P1 | S | — | 🟡 **BUILT 2026-09-07 — data lands with the SIM-469 rebuild.** `build_battedball_pool_artifact` writes `{hand}.pgeom.npy` (the ten `_GEOM_COLS`, NaN preserved so a row with a missing value is exactly neutral) + `pitcher_id` in the meta; `BattedBallPool.pgeom` / `.pitcher_id`, optional, shareable; the loader is back-compatible. Consumer: SIM-472. ~55 MB. |
| **SIM-464** | Home-or-away flag on both pools | Data | P1 | XS | — | 🟡 **PITCH HALF BUILT 2026-09-07 — data lands with the SIM-469 rebuild.** `bat_home` on `sim.pitch_pool` (migration 0023, `inning_topbot = 'Bot'`), exported (NULL → -1 = unknown), `HandPool.bat_home`; consumer = the pitch-draw weight `SIM_PITCH_HOME_OFF_WEIGHT` (1.0 = off exactly, 0.0 = a hard match on the side, unknown rows neutral) now, the SIM-467 cell index's side dimension next. History: `bat_home` landed on `sim.outcome_pool` 2026-08-29 (SIM-491/migration 0019). |
| **SIM-465** | Pitch-count + times-through-the-order columns (window functions) | Data | P2 | S | — | 🟡 **BUILT 2026-09-07 — data lands with the SIM-469 rebuild; the weight is OFF pending its fit.** Columns `pitcher_pitch_count` (pitches BEFORE the PA — the loop snapshots fatigue once per PA) and `times_through_order` (1 + batters faced before the PA // 9 = the live `times_through_order` helper; the SQL is `SQL_PITCHER_PITCH_COUNT` / `SQL_TIMES_THROUGH_ORDER`, unit-tested against it). Consumer: `FullPoolSampler._f_fatigue` — Gaussians on pitch count + TTO (`SIM_FATIGUE_PC_SIGMA` / `SIM_FATIGUE_TTO_SIGMA`), mean 1 per count bucket, unknown rows neutral; one full-pool pass per PA until SIM-467 restricts it to the cell. Fit: plan §8.2. |
...
| **SIM-467** | Extend the bucket index from 12 cells to the full 2,880-cell filter | Perf | P1 | L | — | ⚡ **FLIPPED ON IN PRODUCTION 2026-09-08 (owner go): live n=100 /simulate 31 s warm (30.8 / 31.4 s on two games; 55 s on the first request after boot; 81-90 s before) — the 30-second exit criterion is met AT the line. The receiving kernel went OFF in the same change (lane 2's configuration). Remaining on this ticket: the strikeouts −2.1% per-count fit (the redesign's part F), stage 2 (the batting-side dimension) after the conditioning rebuild, and the fan-out residue (~3 effective workers of 6) as the next perf item.** 🟡 **CODE LANDED + MEASURED 2026-09-07 (see the top banner): 2.62 → 0.86 s/iteration (3.05×), gated OFF pending the 12×500 lane; the cell path is bit-identical inside the cell; widening per decision #19; `scripts/sim467_speed_probe.py` is the measurement.** 📋 **PLANNED 2026-09-04: `docs/audit/2026-09-04-sim467-518-plan.md` (read it first).** 🔲 **OPEN — now ALSO owns the 30-s SLA exit criterion (SIM-436 merged in, 2026-08-29): n=100 /simulate under 30 s, or a documented re-scope.** ~1000× less per-draw work. **This is the answer to the open 30-s SLA**, which the notes wrongly call irreducible. |
| **SIM-468** | New table `sim.steal_opportunity_pool` | Data | P1 | M | — | ✅ **CLOSED 2026-08-17** — the steal OPPORTUNITY pool (migration 0015) landed with the steal draw; steals are back. Was: 🔲 **OPEN.** `stolen_base_pool` holds only attempts — no denominator, so a draw over it attempts 100% of the time. |
| **SIM-469** | Pool + artifact rebuild | Data | P1 | M | 459, 462–468 | 🟡 **SCRIPT READY, RUN BLOCKED 2026-09-07.** Pool-only (the plan's §6.5 ruling: the pitch pool for 2023-2026 + the artifact re-export, ~2 h; NOT the profile recompute — SIM-522's column waits for that). `scripts/sim518_rebuild_pools.py`: migration 0023 → rebuild → independent recomputation of all three columns from `raw.pitches` on 500 random games → export → loader round-trip. Its `duckdb.connect` fails on the writer lock the app's forkserver holds (SIM-524): run it with the app stopped, and note the export overwrites the bundle every running lane reads. The live-source dry check of the new SQL passed (read-only, one 2026 game). |
...
| **SIM-471** | Pitch-outcome draw | ML | P1 | M | SIM-470 | ⛔ **CLOSED-DELIVERED 2026-08-29.** The count-bucketed factorized pitch draw IS this ticket. The framing tier is SIM-517; the fatigue tier is SIM-518 (pitch-count part); prev-pitch is refuted. |
| **SIM-472** | Batted-ball draw with **pitch similarity primary** | ML | P1 | M | 463, 471 | 🟡 **BUILT 2026-09-07, OFF pending its fit (plan §8.1).** SIM-517 already exposed the drawn pitch row (`_pp_last_i`); `FullPoolSampler.last_pitch_geom()` reads its ten geometry values (None on a missing velocity), the loop passes them to `battedball_new_pa(pitch_geom=…)` only when `SIM_BB_PITCH_SIGMA` > 0, and `_f_pitch_similarity` is a Gaussian on the z-scored 10-dim distance inside the SIM-511 base-out cell, mean 1 over complete rows, missing-geometry rows neutral. Needs the SIM-469 rebuild for `pgeom`. The "same row is the batted ball" alternative is recorded as rejected in the plan. |
| **SIM-473** | Advancement draw: the transition mapping | ML | P1 | M | 462, 458 | ⛔ **SUPERSEDED 2026-08-18 by SIM-511/512** (the fielding-transition epic banner at the top). Replaces 7 tuned constants; a runner on 1B currently **cannot advance on any out**. |
| **SIM-474** | Steal draw (attempt + outcome) from the opportunity pool | ML | P1 | M | SIM-468 | ✅ **CLOSED-CERTIFIED 2026-08-30** — the steal draw landed 2026-08-17 and the three steal bands certified with the kernel fit's part 1 (the leverage double-count deleted). Was: 🔲 **OPEN.** Production has had **zero steal attempts** since 2026-06-04. |
...
| **SIM-485** | Hit-by-pitch channel | Data | P2 | S | — | ✅ **CLOSED-DELIVERED 2026-08-18 by the hit-by-pitch outcome (SIM-509)**, certified as a pool band 2026-08-20. Was: 🔲 **OPEN.** Ships with **no** re-sweep — `events` already records it. |
...
| **SIM-487** | ETL parser batch — split `passed_ball_wild_pitch`; add `balk` + `pickoff` | Data | P2 | M | — | ✅ **CLOSED 2026-08-13** — the batched parser changes landed and the re-sweep ran on them. Was: 🔲 **OPEN.** Collect **every** column first; the free window closed 07-30. |
| **SIM-488** | Batched ETL re-sweep (~6 h) | Data | P2 | L | SIM-487 | ✅ **CLOSED 2026-08-13** — the re-sweep ran once for everything (2017-2026, ~6 h, 22,533 games). Was: 🔲 **OPEN.** Run once, for everything. |
| **SIM-489** | Wild-pitch / passed-ball / balk / pickoff channels | Sim | P2 | M | SIM-488 | ✅ **CLOSED-DELIVERED 2026-09-04** — wild pitches and passed balls are the drawn pitch row's got-away fact (the receiving profile, 2026-09-04); pickoffs are the pickoff channel (built 2026-08-18, certified 2026-08-30). Balks are ingested (`raw.play_events`) but not simulated — about 0.02 per team-game, below every band floor; refile with a measured case. Was: 🔲 **OPEN.** ~0.15–0.25 R/team-game with SIM-485. |
| **SIM-490** | Wire the real per-team manager profiles | ML | P2 | S | — | ⤵ **MERGED into the real-manager-profiles ticket (SIM-427) 2026-09-08 (hygiene sweep 2)** — the same deliverable, planned 2026-09-04. Was: 🔲 **OPEN.** Computed, never connected; the model uses a league-flat default. |
| **SIM-491** | Re-validate all six realism flags, **one at a time**, 400 sims × 20 games | Test/CI | P1 | L | 449, 450, 513 | ✅ **CLOSED 2026-08-29 (hygiene sweep 1)** — built with the data live; the fits and enables moved to the kernel-bandwidth fit, closed 2026-09-03. Was: 🔲 **OPEN.** All five were enabled together at 3–4 games, the day after that bar was set. **Owner ruling 2026-08-19: this ticket owns ALL nudge/flag re-validation against the SIM-510→513 transition draws** — SIM-412 home-… |
| **SIM-492** | Calibration refit + multi-season win-probability curve | ML | P1 | M | all | ⤵ **MERGED into the calibration payoff (SIM-429) 2026-09-08 (hygiene sweep 2)** — the refit ran 2026-08-16 on the rebuilt data (win-probability ECE 0.038; the walk prop bettable); the strikeout-prop refit and the multi-season curve are that ticket's remaining work, now 3× cheaper per lane. Was: 🔲 **OPEN.** The live curve was fitted on 60 games of one season, on the **pre-fix** run environment. |
| **SIM-493** | Decompose `sim_loop.py` | Sim | P3 | L | 481, 486 | 🔲 **OPEN (P3).** `sim_loop.py` is 3,487 lines after the per-tile deletion (was 4,557); decompose once, on the final shape, after the cell-index and conditioning rulings land. Was: 🔲 **OPEN.** Deferred deliberately — the architecture work deletes ~400 lines and SIM-486 several hundred more. Decompose once, on the final shape. |
...
| ID | Title | Type | Pri | Size | Depends-on | Status |
|---|---|---|---|---|---|---|
| SIM-444 | Execute the 10-phase remediation plan | Epic | P1 | XL | — | ✅ **CLOSED-SUPERSEDED 2026-09-08 (hygiene sweep 2).** Executed through the 2026-08-10 sim-loop remediation programme and the 2026-08-13→16 data rebuild: the acceptance lane and the harness (phase 1), the free fixes (2), the re-sweep, recompute, artifacts and refit (7-8), the certifying lanes (9). Phase 10 — the terminal CLV read — is the calibration payoff (SIM-429). Phase 0's datastore backups were never ticketed: filed today as SIM-525. Was: 🔲 **OPEN.** Phases: 0 stop irreversible loss (forward odds capture is dead wiring; there is no backup of any datastore) → 1 build the instruments (the validation harness provably cannot exercise the f… |
