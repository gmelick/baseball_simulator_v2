...
| the outcome class | the pool build reads it from the MLB feed's pitch code (`SQL_OUTCOME_TYPE`). **A foul tip or a foul bunt with two strikes is strike three** (`swinging_strike`, SIM-553, 2026-09-25); below two strikes it is a foul. The count machine then ends the plate appearance. Before SIM-553 the pool coded it as a foul the machine plays on, and the count chain put the simulator's loss at 4.4% of its strikeouts. An unknown code fails the pool build. The pool was rebuilt on this coding on 2026-09-25 (builder `sim553.1`, all ten seasons); `tests/acceptance/test_sim553_pool_label_check.py` checks the pool's labels against real plate appearances (within 0.5%) |

### ⑥ The batted ball — the fence stage, then `battedball_draw`

| | |
|---|---|
| the fence stage (ON) | for an air ball, the born ball's carry against the live park's real fence decides home run / not before any draw (`park_geometry.json`; the band is 0 feet). **The carry is read in the live park's air** (2026-09-22, `SIM_CARRY_OFFSET`): the ball's own distance plus the live park's carry offset minus the offset of the park it was hit in — one number per park from the pool's own home runs (Coors +21 ft, Kauffman +10, the rest within ±7), stored in the geometry document; the same number is the ball's distance in the draw below. The fence line is one number per park, ten-degree spray sector (eleven sectors over ±55°, so a ball down the line has its own) and **season group** (a wall that moved — Camden Yards' left field, 2025 — opens a new group; the live season picks it). A thin sector rests on the park's published fence distances plus the league's offset, blended toward its own balls at ten home runs. **Certified 2026-09-21 (SIM-478/479/480):** on every real air ball of the pool window the call is right on 91.7% of the balls it sends over and misses 7.8% of the home runs; `scripts/sim478_fence_check.py` re-runs it in two minutes. |
| pool | one row per real ball in play with its whole base-out transition |
| hard filters | the exact base-out cell (the drawn row must be legal here) · the born ball's **class** (ground ball, line drive, fly ball, popup, bunt) · for a wall-zone air ball (300+ ft in the live air) called short: the **wall margin** — rows whose ball landed within **15 ft** of the same distance from THEIR park's fence as the born ball is from the live one (`SIM_BB_MARGIN_BAND`, 2026-09-22; the cell's rows when fewer than 20 remain) · the **batting side** (the home weight is 0.0: rows of the other side are excluded — this is where home-field advantage comes from) |
| weights | recency · a Gaussian on the born ball (exit velocity, launch angle, spray, distance; z-scored on the **born ball's class** — a fly ball against the fly balls' spread, 64 ft of distance, not the whole pool's 135 — with the exponent per feature, `SIM_BB_BORN_PER_FEATURE`; **bandwidth 1.0**; 2026-09-22) · the live batter's **batter similarity** (**power 1**; fitted 4) · a Gaussian on the situation's soft dimensions (balls, strikes, inning, margin; bandwidth 2.0) · the **platoon**: rows whose pitcher hand does not match the live matchup × **0.6** · the live defender at the row's fielded position: **fielder similarity** to the row's fielder (**power 1**; fitted 1.2, per position) · the park: a Gaussian on the run factor (**bandwidth 0.02**), only for wall-zone balls |
