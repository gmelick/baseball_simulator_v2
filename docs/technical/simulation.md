...
| `market_probability(runs, market_type, *, line, side="home")` | The simulator's probability of the FIXED reference side (HOME on a side / run-line market, OVER on a total) — the number the accuracy comparison pairs against the closing line. SIM-549: a run line also takes `side="away"`, with `line` the away team's own spread — the away bet of a run line the book lists as two separate bets (the away leg of `cover_probabilities` at the mirrored home spread). | `scripts/clv_backtest.py` | — |
| `market_outcome(actual, market_type, *, line, side="home")` | The 0/1 label of the reference side on ONE real result, or None on a push (a total on the line, an adjusted margin of zero, a tied segment on a two-way side market). A tied segment on a THREE-way market is a home loss, not a push — the draw was a priced outcome. SIM-549: a run line's away bet at its own spread (`side="away"`) covers when away − home + line > 0. | `scripts/clv_backtest.py` | — |
