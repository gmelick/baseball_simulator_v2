...
| Function / class | What it does | Called from | Depends on |
|---|---|---|---|
| `BOARDS` | The registry: ten boards — the batter swing/stance family (`bat_tracking`, `swing_path`, `batting_stance`; SIM-529), the fielding boards (`arm_strength`, `baserunning`, `poptime`, `catcher_throwing`, `first_base_receiving`; SIM-530) and the running-game boards (`basestealing`, `pitcher_running_game`; SIM-531). `BATTER_BOARDS` / `FIELDING_BOARDS` / `RUNNING_BOARDS` are the group aliases `--boards` accepts. | `resolve_boards`, `scripts`, the profile builders (by table) | — |
