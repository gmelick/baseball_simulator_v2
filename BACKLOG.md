...
| ID | Title | Type | Pri | Size | Depends-on | Status |
|---|---|---|---|---|---|---|
| **SIM-514** | Post-transition scoreboard findings: DP +8.3%, K -2.5%, SB -7.6%, 3B +11.9% | Sim | P1 | M | SIM-513 | 🔲 **OPEN — diagnosis first, measurement before tuning. The worked plan: `docs/audit/2026-08-19-sim429-514-491-diagnosis-plan.md` §2.** The four reds form one interacting system around traffic: (a) **DP +8.3%** — hypothesis: the BB surplus (+11.7%, SIM-429) puts extra runners on 1B, and every extra runner is a DP chance; decompose DP per-opportunity (runner-on-1B, <2 outs) vs per-game before touching anything. (b) **K -2.5%** (was -1.6%) — more DPs end innings sooner → fewer PAs; decompose K/PA vs PA/game. (c) **SB -7.6%** (was +5.7% HIGH pre-epic) — DPs remove the runners who would steal; decompose attempts per opportunity-pitch (the SIM-468 denominator) vs opportunities per game. If all three normalize per-opportunity, the ONE upstream defect is SIM-429's BB and these rows close with it. (d) **3B +11.9%** (+0.015/tg absolute; was +4.9%) — not traffic; check the hard-cell draw's event mix per cell vs the pool's own, and whether S5 double→3B stretches double-count with drawn-triple rows. |
...
| **SIM-511** | The transition fielding draw — the drawn row IS the play | Sim | P1 | L | SIM-510 | 🟢 **LANDED 2026-08-19 with SIM-512 (one combined change); the certification lane adjudicates the bands under SIM-513.** The spec as filed: Replace `_full_pool_fielding`'s event-only draw + out-inference (`outs = 0 if rh else 1`) with a draw whose HARD filter includes the base-out state (runner config × outs — 24 cells, ALL common; **NO widening, owner ruling 2026-08-19**: the base-out filter is essential and never relaxes, the count is never a hard filter, and an empty cell raises as a data defect): the drawn row supplies event, outs, who is out, and forced/automatic movement — normalized station-to-station on the five discretionary scenarios. DELETE: the phantom-DP guard (~sim_loop:1521), the discarded-`result_outs` note, `_full_pool_out_advancement`'s tuned constants for forces/productive outs, and the `constants.py:177` field_error→single alias (the row carries batter-reached truth — the SIM-496 fix) PLUS the `RUN_VALUES` linear-weight NUMBERS (owner 2026-08-19: only tests read them — the ledger is RE24-only; the canonical-outcome vocabulary + alias map STAY). Doubled-off runners, FCs, infield hits, compound errors all arrive via rows. `runners_retired` becomes real (the SIM-494 out-count-vs-bodies check ships as its regression test). Strict xfails on DP / H / ROE_reached come OUT when their bands land (their own instruction). |
| **SIM-512** | The five-scenario advancement draw | Sim | P1 | L | SIM-510, SIM-511 | 🟢 **LANDED 2026-08-19 with SIM-511 (one combined change).** The spec as filed: Per-runner attempt→outcome draws from the SIM-510 opportunity pools, lead-first with can't-pass occupancy; if the lead does not attempt, no trailing draws — EXCEPT the batter-stretch draw (advance-on-the-throw is folded into it, accepted approximation). Outcomes: hold / advance-safe / advance-out / advance-plus-error. All advancement outs are TAG plays → Rule 5.08 timing: runs that crossed before a trailing tag-out COUNT (lead-first resolution gives this nearly free). A tag from 3B that scores on a fly out labels the play `sacrifice_fly` post-hoc (AB/RBI per SIM-312). Delete `_tag_rate`, the 0.28/0.30/0.35 constants, `_extra_advance`. |
...
| **SIM-476** | The kernel-bandwidth fit (re-scoped 2026-08-28) | ML | P1 | L | — | 🔲 **OPEN — PLANNED: `docs/audit/2026-08-28-sim476-fit-plan.md` (read it first; the SIM-470 dependency is DISSOLVED — the draws landed as factorized kernels, so SIM-476 fits the knobs that exist).** The fit principle: a kernel is correct when the sim's frequencies CONDITIONED on what the kernel reads match the pool's own conditional frequencies — fit per-opportunity (cheap), verify at game level (once). Part 1: the steal attempt weighting (`steal_sigma`/`steal_score_sigma`) against per-cell attempts/opportunity — the certified lane's only frequency red (2B −15%); localize per cell first; then add STEAL_ATT_OPP/SAFE pool bands and retire the SB/CS 2025-era asserts. Part 2: `home_off_weight` against the pool's own bat_home conditional event mix, THEN the 13,365-game home_win_pct lane once. Part 3: `park_sigma` + `fielder_sigma` the same conditional way (their legacy flips DELETE on enable). Fit only what is red or off — `sit_sigma`/`batter_sigma`/`adv_*` drive certified-green channels and are not targets. |
...
| **SIM-491** | Re-validate all six realism flags, **one at a time**, 400 sims × 20 games | Test/CI | P1 | L | 449, 450, 513 | 🔲 **OPEN.** All five were enabled together at 3–4 games, the day after that bar was set. **Owner ruling 2026-08-19: this ticket owns ALL nudge/flag re-validation against the SIM-510→513 transition draws** — SIM-412 home-field (reads 0.5077; re-tune vs the 0.5428 centre), SIM-411 park, SIM-425b fielder-RBF (a pre-draw out↔single flip must stay consistent with a drawn transition row — the flip may need to become a re-draw or a conditioning weight). Run AFTER SIM-513 lands; SIM-513 contains no validation. **The worked rebuild designs (home flag column + platoon-style reweight / park-factor kernel on the per-row venue / fielder-quality factor over the OAA embedding): `docs/audit/2026-08-19-sim429-514-491-diagnosis-plan.md` §3.** |
