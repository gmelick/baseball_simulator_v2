...
| ID | Title | Type | Pri | Size | Depends-on | Status |
|---|---|---|---|---|---|---|
| **SIM-515** | Replace the hand-tuned IBB formula with a play_events draw | Sim | P1 | M | — | 🔲 **OPEN — the largest, cheapest BB win (−~0.20/tg).** Replace `_should_issue_ibb` (sim_loop.py, tendency × leverage vs rng, per-pitch re-roll) with the architecture-rule treatment: a per-PA draw whose rate comes from real PAs in the hard-filtered IBB cell (1B open × RISP × outs × late-and-close class). `raw.play_events` holds 6,160 `intent_walk` events (2017-2026) with `runners_state`, `outs_before`, `inning`, `bat_score`/`fld_score`, batter/pitcher ids — the denominator is PAs in the same cells from `raw.pitches`. Decide the delivery (a small rates table in the engine-artifact bundle, or an opportunity pool cloning SIM-468) before building. After it lands: re-run the 12×471 lane — expect BB −~0.20/tg and DP/SB opportunities to normalize with it. |
