...
> | SIM-429 | Re-calibration + CLV backtest over the rewired loop — **IN PROGRESS**: full-pool flipped on as the production default (`SIM_FULL_POOL=1` in compose) after a 100-sim realism validation (rate stats within ~7% of MLB; runs ~13% low = the hits→runs conversion gap). REMAINING: engine-backed baserunner advancement (SIM-425) → run-conversion recalibration → CLV backtest | Validation | P1 | L | ML Engineer + Betting Analyst | SIM-424, SIM-425, SIM-426, SIM-427 |
>
