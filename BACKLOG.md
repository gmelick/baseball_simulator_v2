...
*Owner: Product Manager (Agent 1) · Last updated: 2026-05-28 (six closures today: SIM-403 + SIM-403b + SIM-404 + SIM-409 + SIM-412 + SIM-414. 🚀 PHASE 6 Sprint 1 COMPLETE — all 13 P0s + SIM-410. Sprint 2 (P1 frontend build) COMPLETE — SIM-391→401. Hardening batch COMPLETE — SIM-415/416/417/418/419/420. Remaining: perf SIM-430; SIM-402/406/407/408 closed 2026-05-30; P2 sim-realism SIM-411 + SIM-413 (both play-pool-rebuild-blocked))*
...
>
> **⚠ Reality check (QA-confirmed):** the pre-existing Phase-6 tickets SIM-127–131 cite parent tickets SIM-108/109/112/122–126 that **don't exist**; `frontend/{components,graphics,pages}/` are **empty dirs**; there is no build tooling or API→UI serving path. SIM-382 backfills the chain. **Next free ID: SIM-433** (SIM-430 = full-pool `/simulate` throughput / 2s-30s SLA perf ticket, filed 2026-05-30 from the SIM-402 live re-measure — see the SIM-402 row: per-game ~2.2s full-pool cost + the n-iteration fan-out that's serial at 1 worker and OOM-deadlocks at 10; SIM-431 = the Python-3.13 migration (CLOSED 2026-05-30); SIM-430 per-game cost cut 1.21x (full-pool per-PA caching, 1.85s→1.53s/game warm, 2026-05-30; SLA not yet met — fan-out half open) — the worker fan-out/OOM half remains; SIM-432 = the validate-props win-prob-curve live run (follow-up); SIM-422→429 = the similarity-engine-wiring epic; SIM-421 = the P3 full-market-projection enhancement).
>
> **SIM-432 (P1, NEW 2026-05-31) — calibrator + validate_props vs live-schema reconciliation (unblocks SIM-406 calibration + the SIM-407 win-prob curve; subsumes the earlier 'SIM-432 = validate-props live run').** SIM-406/407 were code-complete but NEVER run against the live DB; running them surfaces a cascade of mismatches (scripts assume an un-trimmed schema; live `derived.*`/`raw.*` are SIM-408-trimmed). Found: (a) batter calibrator selected `xba`/`xslg` -> FIXED (`_opt` guard, commit ee1188f); (b) pitcher sub-calibrator imports `RESULT_FEATURES` but `pitcher_similarity` now exports ONLY `COMMAND_FEATURES` -> NOT fixed; (c) batter query needs `first_pitch_take_rate`/`max_exit_velo`/the platoon `*_vs_r` block -> ABSENT in live `derived.batter_season_metrics`; (d) `validate_props._fetch_final_games` selects `raw.games.home_score` -> ABSENT in live Postgres; the 6 non-batter sub-calibrators likely diverge too. Needs a SIM-408-style trim/guard pass; app correctly runs identity calibration until then.
>
> | Tier | Tickets |
> |---|---|
