...
| **SIM-513** | Retire the legacy paths + certify the finalized loop | Sim/Test | P1 | M | SIM-511, SIM-512 | 🔶 **CERTIFICATION COMPLETE 2026-08-19** (the 12×471 lane ran end to end; the scoreboard is read and filed — see the top banner; SIM-514 carries the findings). The H/DP/ROE_reached strict xfails are DELETED (their own instruction, validated: H passed, DP/ROE_reached measured real). The SIM-455/SIM-484 filings are VERIFIED (both claims hold; both stay open; SIM-484's SIM-496-era framing corrected). **Remaining before close: retire the SIM-349 sac-fly-intent nudge** (superseded; its call site is bypassed on the transition path — the deletion spans `_apply_sac_fly_bias`, `_maybe_sac_fly_intent`, `ManagerContext.sac_fly_intent`, and the SIM-349 intent tests). **NO validation in this ticket (owner ruling 2026-08-19)** — SIM-491 owns re-validating the upstream nudges (SIM-412 home-field vs the 0.5428 centre, SIM-411 park, SIM-425b fielder-RBF) against the new draws, AFTER this ticket lands. Golden-fixture regen (intentional model change), full 12×471 lane on the 2025 bands, re-read the WHOLE scoreboard (DP/ROE fixes move R both ways), and re-check the CS/3B/ROE overdispersion (sim sd > sd_ref) — correct DP/ROE mechanics may be its cause. Also verify-and-close the stale SIM-455/SIM-484 filings while in the file. |
