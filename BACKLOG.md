...
>
> **⚠ Reality check (QA-confirmed):** the pre-existing Phase-6 tickets SIM-127–131 cite parent tickets SIM-108/109/112/122–126 that **don't exist**; `frontend/{components,graphics,pages}/` are **empty dirs**; there is no build tooling or API→UI serving path. SIM-382 backfills the chain. **Next free ID: SIM-432** (SIM-430 = full-pool `/simulate` throughput / 2s-30s SLA perf ticket, filed 2026-05-30 from the SIM-402 live re-measure — see the SIM-402 row: per-game ~2.2s full-pool cost + the n-iteration fan-out that's serial at 1 worker and OOM-deadlocks at 10; SIM-431 = the Python-3.13 migration (CLOSED 2026-05-30: CI+Docker+local unified on 3.13, numpy 2.x); SIM-422→429 = the similarity-engine-wiring epic; SIM-421 = the P3 full-market-projection enhancement).
>
> | Tier | Tickets |
> |---|---|
