...
def _make_engine(profiles_matrix: np.ndarray, meta_rows: list[dict],
    ...
    from similarity.engines.pitch_pitch_similarity import (
        PitchPitchSimilarityEngine, PitchNormalizer,
        NearestPitch, FEATURE_DIM,
    )
    import faiss
    ...
    engine._index_meta = [
        NearestPitch(
            ...
            pitcher_id=r["pitcher_id"], batter_id=r["batter_id"],
            distance=0.0, outcome_type=r["outcome_type"],
        ) for r in meta_rows
    ]
...
class TestPitchPitchEngineBasic(unittest.TestCase):
    ...
    def test_query_returns_k_results_sorted_ascending(self):
        ...
        q = PitchVector(
            velo=94.0, ivb=18.0, hb=-3.0, spin_rate=2400.0, spin_axis=210.0,
            release_x=-1.5, release_z=5.9, release_ext=6.5,
    ...
    def test_empty_engine_returns_empty(self):
        from similarity.engines.pitch_pitch_similarity import (
            PitchPitchSimilarityEngine, PitchVector,
        )
        engine = PitchPitchSimilarityEngine.__new__(PitchPitchSimilarityEngine)
        engine._index = None
        engine._index_meta = []
...
class TestPitchPitchFeatureContract(unittest.TestCase):
    ...
    def test_recency_boost_replicates_recent_seasons(self):
        from similarity.engines.pitch_pitch_similarity import (
            PitchPitchSimilarityEngine, NearestPitch, FEATURE_DIM,
        ...
        for i in range(30):
            ...
            meta.append(NearestPitch(
                pitch_id=i, game_pk=0, season=season,
                pitcher_id=0, batter_id=0, distance=0.0,
                outcome_type="ball",
            ))
