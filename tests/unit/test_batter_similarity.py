...
def _make_profile(
    ...
    return BatterProfile(
        ...
        discipline_vec=np.array(
            discipline or [0.60, 0.28, 0.68, 0.78, 0.22, 0.20, 0.10], dtype=np.float64
        ),
        batted_ball_vec=np.array(
            batted_ball or [0.42, 0.36, 0.40, 0.25, 89.0, 12.0, 0.38, 0.07], dtype=np.float64
        ),
        power_vec=np.array(power or [0.04, 0.260, 0.430, 108.0], dtype=np.float64),
        platoon_vs_l_vec=np.array(
            platoon_l or [0.30, 0.66, 0.24, 0.09, 0.22, 0.44, 0.06], dtype=np.float64
        ),
        platoon_vs_r_vec=np.array(
            platoon_r or [0.27, 0.70, 0.20, 0.11, 0.18, 0.40, 0.08], dtype=np.float64
        ),
...
class TestScoreProperties(unittest.TestCase):
    ...
    def test_all_scores_in_range(self):
        ...
        for r in results:
            ...
            for sub in [r.discipline_score, r.batted_ball_score, r.platoon_score, r.power_score]:
...
class TestPlatoonAwareQuery(unittest.TestCase):
    ...
    def test_vs_hand_changes_scores(self):
        ...
        self.assertTrue(any(d > 0.001 for d in diffs), "vs_hand should change at least some scores")
    ...
    def test_platoon_divergent_batter_scored_differently_per_hand(self):
        ...
        self.assertGreater(
            score_good, score_bad, "Similar platoon profile should score higher than dissimilar"
        )
