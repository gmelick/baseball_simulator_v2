...
def _build_steal_engine():
    from similarity.engines.baserunner_steal_similarity import (
        BaserunnerStealSimilarityEngine, BaserunnerStealProfile,
        FeatureNormalizer, StealPartition, WeightedRBFSimilarity,
    ...
    engine._normalizer = FeatureNormalizer(); engine._normalizer.fit(profiles)
    engine._partition = StealPartition(); engine._partition.build(profiles, engine._normalizer)
...
def _build_catcher_engine():
    ...
    engine._normalizer = FeatureNormalizer(); engine._normalizer.fit(profiles)
    engine._partition = CatcherPartition(); engine._partition.build(profiles, engine._normalizer)
...
def _build_pitcher_steal_engine():
    from similarity.engines.pitcher_steal_similarity import (
        PitcherStealSimilarityEngine, PitcherStealProfile,
        FeatureNormalizer, PitcherStealPartition, WeightedRBFSimilarity,
    ...
    engine._normalizer = FeatureNormalizer(); engine._normalizer.fit(profiles)
    engine._partition = PitcherStealPartition(); engine._partition.build(profiles, engine._normalizer)
...
def _build_manager_engine():
    from similarity.engines.manager_similarity import (
        ManagerSimilarityEngine, ManagerProfile,
        FeatureNormalizer, ManagerPartition, WeightedRBFSimilarity,
    ...
    engine._normalizer = FeatureNormalizer(); engine._normalizer.fit(profiles)
    engine._partition = ManagerPartition(); engine._partition.build(profiles, engine._normalizer)
...
def _build_situation_engine():
    from similarity.engines.situation_similarity import (
        SituationSimilarityEngine, SituationVector, SituationNormalizer,
        NearestSituation, FEATURE_WEIGHTS, SCORE_DIFF_CLIP,
    )
    ...
    situations = [
        SituationVector(
            inning=int(rng.integers(1, 10)), top_or_bottom=int(rng.integers(0, 2)),
            outs=int(rng.integers(0, 3)),
            runner_on_1b=int(rng.integers(0, 2)), runner_on_2b=int(rng.integers(0, 2)),
    ...
    for i, sv in enumerate(situations):
        ...
        index_meta.append(NearestSituation(
            play_id=str(i), game_pk=700000 + i, distance=0.0,
            inning=sv.inning, outs=sv.outs, runners=runners,
            leverage_index=sv.leverage_index, score_differential=score_diff_clipped,
        ))
    ...
    normalizer = SituationNormalizer(); normalizer.fit(raw_matrix)
...
def _generate_situation_fixture(engine, dry_run: bool, force: bool):
    ...
    query_vectors = [
        SituationVector(inning=7, top_or_bottom=0, outs=1, runner_on_1b=0, runner_on_2b=1,
                        runner_on_3b=0, score_differential=0.0, leverage_index=2.1,
                        pitcher_pitch_count=55, batter_pa_count=2, park_factor_runs=1.02),
        SituationVector(inning=3, top_or_bottom=1, outs=0, runner_on_1b=1, runner_on_2b=0,
                        runner_on_3b=0, score_differential=-1.0, leverage_index=0.8,
                        pitcher_pitch_count=30, batter_pa_count=1, park_factor_runs=0.95),
        SituationVector(inning=9, top_or_bottom=0, outs=2, runner_on_1b=1, runner_on_2b=1,
                        runner_on_3b=1, score_differential=1.0, leverage_index=3.2,
                        pitcher_pitch_count=75, batter_pa_count=3, park_factor_runs=1.05),
        SituationVector(inning=1, top_or_bottom=0, outs=0, runner_on_1b=0, runner_on_2b=0,
                        runner_on_3b=0, score_differential=0.0, leverage_index=0.5,
                        pitcher_pitch_count=0, batter_pa_count=0, park_factor_runs=1.00),
        SituationVector(inning=5, top_or_bottom=1, outs=1, runner_on_1b=0, runner_on_2b=0,
                        runner_on_3b=0, score_differential=2.0, leverage_index=0.7,
                        pitcher_pitch_count=45, batter_pa_count=2, park_factor_runs=0.98),
    ]
...
def _write_fixture(name: str, data: dict, dry_run: bool, force: bool) -> None:
    ...
    print(f"  WROTE {path.name}  ({len(data['queries'])} queries, {data.get('n_profiles', data.get('n_situations', '?'))} profiles)")
