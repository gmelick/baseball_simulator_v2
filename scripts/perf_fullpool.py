...
t0 = time.time()
cols = ("velo,ivb,hb,spin_rate,spin_axis,release_x,release_z,release_ext,plate_x,plate_z,"
...
N = len(d["velo"])
geom = np.ascontiguousarray(np.stack([d[c] for c in
        ["velo","ivb","hb","spin_rate","spin_axis","release_x","release_z","release_ext","plate_x","plate_z"]],
        axis=1).astype(np.float32))
sit = np.ascontiguousarray(np.stack([d[c] for c in
        ["count_balls","count_strikes","outs","runners_state","inning","score_diff"]], axis=1).astype(np.float32))
geom = np.nan_to_num(geom); sit = np.nan_to_num(sit)
...
pitcher_sim = np.random.default_rng(0).random(len(uniq)).astype(np.float32)  # placeholder per-pitcher score
batter_sim_const = np.float32(0.5)                          # placeholder (batter factor is a per-PA gather, ~= pitcher cost)
print(f"[load] hand={HAND} N={N:,} geom={geom.nbytes/1e6:.0f}MB sit={sit.nbytes/1e6:.0f}MB  in {time.time()-t0:.2f}s")
...
cdf = pa_setup()
print(f"[pa_setup total]  {timeit(pa_setup):7.1f} ms   (situation+batter+cumsum; base cached/half-inning)")
...
print(f"=== per 100-game batch (1 worker): {100*game_ms/1000:.0f} s  (SLA = 30 s, but batch parallelizes across workers) ===")
