...
def _framing_extremes():
    ...
    print(f"  min={arr.min():+.4f}  p10={np.percentile(arr,10):+.4f}  med={np.median(arr):+.4f}"
...
async def _run():
    ...
    await conn.close()
    base = {
        "away_lineup": list(st.away_lineup), "home_lineup": list(st.home_lineup),
        "season": int(st.season), "pitcher_id": int(st.pitcher_id or 0),
        "bat_hand": st.bat_hand, "bat_hands": dict(st.bat_hands),
        "throw_hands": dict(st.throw_hands), "home_pitcher_id": st.home_pitcher_id,
        "away_pitcher_id": st.away_pitcher_id, "k": 25, "max_innings": 12,
    }
    ...
    for label, cid in (("BEST framer", best_id), ("WORST framer", worst_id)):
        ...
        print(f"  {label:14s} (id={cid}): K/game={statistics.mean(ks):.2f}  BB/game={statistics.mean(bbs):.2f}")
